# receiver.py - Modified: added thread lock protection for dBgrid_main, store PyAudio instance
import threading
import numpy as np
import wave
import time
from time_utils import global_time_utils, Ticker
import os
import pyaudio
import pickle
from databases import call_hashes, add_call_hashes
...
def decode_free_text(bits71):
    if bits71 is None or bits71 < 0:
        return None
    remaining = bits71
    chars = []
    for i in range(13):
        idx = remaining % 42
        chars.append(FREE_TEXT_CHARSET[idx])
        remaining //= 42
    text = ''.join(reversed(chars))
    return text.rstrip()


def pack_free_text(text):
    if len(text) > 13:
        text = text[:13]
    text = text.ljust(13, ' ')
    indices = []
...
def get_bitfields(bits, lengths):
    ...
    for n in lengths:
        mask = (1 << n) - 1
        fields.append(remaining & mask)
        remaining >>= n
    return (*fields, remaining)


def unpack(bits):
    result = get_bitfields(bits, [3])
    if len(result) >= 2:
        i3 = result[0]
        bits74 = result[1]
    else:
    ...
    if i3 == 0:
        ...
        if n3 == 0:
            text = decode_free_text(bits71)
            if text:
                return ('FREE_TEXT', text)
            return None
        return None
    if i3 == 6:
        return unpack_transparent(bits74)
    if i3 == 1:
        return unpack_std(bits74, i3)
    return None


def unpack_transparent(bits74):
    b72 = bits74 >> 2
    f2 = bits74 & 0x3
    format_map = {0: 'RAW_BINARY', 1: 'USER_DEFINED_1', 2: 'USER_DEFINED_2', 3: 'USER_DEFINED_3'}
    format_name = format_map.get(f2, f'FORMAT_{f2}')
    byte_data = b72.to_bytes(9, byteorder='big')
    byte_data_trimmed = byte_data.rstrip(b'\x00')
    if len(byte_data_trimmed) == 0:
        byte_data_trimmed = b'\x00'
    return ('TRANSPARENT', format_name, f'0x{b72:018x}', str(b72), bits74, f2, byte_data_trimmed.hex(), b72)
...
def unpack_std(bits74, i3):
    ...
    call_a = call_29(ca29, i3)
    call_b = call_29(cb29, i3)
    if call_a and call_b and grid_rpt:
...
def call_29(call_int29, i3):
    ...
    if call_int28 < 21443:
        ...
        for i in range(4):
            idx = x % 27
            txt = " ABCDEFGHIJKLMNOPQRSTUVWXYZ"[idx] + txt
            x //= 27
        txt = txt.rstrip()
        return f"CQ {txt}" if txt else "CQ"
    if call_int28 >= 2063592 + 4194304:
        call = standard_call28(call_int28, i3)
        if portable_rover:
            call = call + ('/P' if i3 == 2 else '/R')
        add_call_hashes(call)
        return call
    hash_val = call_int28 - 2063592
    for m in [22, 12, 10]:
        result = call_hashes.get((hash_val & ((1 << m) - 1), m), None)
        if result:
            if portable_rover:
                result = result + ('/P' if i3 == 2 else '/R')
            return result
    return '<....>'
...
class LdpcDecoder:
    def __init__(self):
        self.CV6idx = np.array(
            [[4, 31, 59, 92, 114, 145], [5, 23, 60, 93, 121, 150], [6, 32, 61, 94, 95, 142],
             [5, 31, 63, 96, 125, 137], [8, 34, 65, 98, 138, 145], [9, 35, 66, 99, 106, 125],
             [11, 37, 67, 101, 104, 154], [12, 38, 68, 102, 148, 161], [14, 41, 58, 105, 122, 158],
             [0, 32, 71, 105, 106, 156], [15, 42, 72, 107, 140, 159], [10, 43, 74, 109, 120, 165],
             [7, 45, 70, 111, 118, 165], [18, 37, 76, 103, 115, 162], [19, 46, 69, 91, 137, 164],
             [1, 47, 73, 112, 127, 159], [21, 46, 57, 117, 126, 163], [15, 38, 61, 111, 133, 157],
             [22, 42, 78, 119, 130, 144], [19, 35, 62, 93, 135, 160], [13, 30, 78, 97, 131, 163],
             [2, 43, 79, 123, 126, 168], [18, 45, 80, 116, 134, 166], [11, 49, 60, 117, 118, 143],
             [12, 50, 63, 113, 117, 156], [23, 51, 75, 128, 147, 148], [20, 53, 76, 99, 139, 170],
             [34, 81, 132, 141, 170, 173], [13, 29, 82, 112, 124, 169], [3, 28, 67, 119, 133, 172],
             [51, 83, 109, 114, 144, 167], [6, 49, 80, 98, 131, 172], [22, 54, 66, 94, 171, 173],
             [25, 40, 76, 108, 140, 147], [26, 39, 55, 123, 124, 125], [17, 48, 54, 123, 140, 166],
             [5, 32, 84, 107, 115, 155], [8, 53, 62, 130, 146, 154], [21, 52, 67, 108, 120, 173],
             [2, 12, 47, 77, 94, 122], [30, 68, 132, 149, 154, 168], [4, 38, 74, 101, 135, 166],
             [1, 53, 85, 100, 134, 163], [14, 55, 86, 107, 118, 170], [22, 33, 70, 93, 126, 152],
             [10, 48, 87, 91, 141, 156], [28, 33, 86, 96, 146, 161], [21, 56, 84, 92, 139, 158],
             [27, 31, 71, 102, 131, 165], [0, 25, 44, 79, 127, 146], [16, 26, 88, 102, 115, 152],
             [50, 56, 97, 162, 164, 171], [20, 36, 72, 137, 151, 168], [15, 46, 75, 129, 136, 153],
             [2, 23, 29, 71, 103, 138], [8, 39, 89, 105, 133, 150], [17, 41, 78, 143, 145, 151],
             [24, 37, 64, 98, 121, 159], [16, 41, 74, 128, 169, 171]], dtype=np.int16)
        self.CV7idx = np.array(
            [[3, 30, 58, 90, 91, 95, 152], [7, 24, 62, 82, 92, 95, 147], [4, 33, 64, 77, 97, 106, 153],
             [10, 36, 66, 86, 100, 138, 157], [7, 39, 69, 81, 103, 113, 144], [13, 40, 70, 87, 101, 122, 155],
             [16, 36, 73, 80, 108, 130, 153], [44, 54, 63, 110, 129, 160, 172], [17, 35, 75, 88, 112, 113, 142],
             [20, 44, 77, 82, 116, 120, 150], [18, 34, 58, 72, 109, 124, 160], [6, 48, 57, 89, 99, 104, 167],
             [24, 52, 68, 89, 100, 129, 155], [19, 45, 64, 79, 119, 139, 169], [0, 3, 51, 56, 85, 135, 151],
             [25, 50, 55, 90, 121, 136, 167], [1, 26, 40, 60, 61, 114, 132], [27, 47, 69, 84, 104, 128, 157],
             [11, 42, 65, 88, 96, 134, 158], [9, 43, 81, 90, 110, 143, 148], [29, 49, 59, 85, 136, 141, 161],
             [9, 52, 65, 83, 111, 127, 164], [27, 28, 83, 87, 116, 142, 149], [14, 57, 59, 73, 110, 149, 162]],
            dtype=np.int16)
...
class AudioIn:
    def __init__(self, max_freq, wav_files=None):
        ...
        self.stream = None
        self.pya_instance = None          # [FIX] Store PyAudio instance
        self.lock = threading.Lock()      # [FIX] Lock to protect dBgrid_main read/write
    ...
    def start_streamed_audio(self, input_device_idx):
        self.pya_instance = pyaudio.PyAudio()  # [FIX] Store as member variable
        self.stream = self.pya_instance.open(
            format=pyaudio.paInt16, channels=1, rate=SAMP_RATE,
            input=True, input_device_index=input_device_idx,
            frames_per_buffer=int(SAMP_RATE / (SYM_RATE * HPS)),
    ...
    def sync_pointer_to_wall_clock(self):
        if self.wav_files is None:
            corrected_time = global_time_utils.corrected_time()
            self.dBgrid_main_ptr = int(corrected_time * SYM_RATE * HPS) % HOPS_PER_GRID
...
class Candidate:
    ...
    def demap(self, dBgrid_main, target_params=(3.3, 3.7)):
        ...
        pmax = np.max(dBgrid)
        self.snr = np.clip(int(pmax - np.min(dBgrid) - 62), -24, 24)
    ...
    def validate(self, msg_tuple):
        if msg_tuple and len(msg_tuple) > 0 and msg_tuple[0] == 'FREE_TEXT':
            free_text = msg_tuple[1] if len(msg_tuple) > 1 else ''
            return f"[Free Text] {free_text}"
        if msg_tuple and len(msg_tuple) > 0 and msg_tuple[0] == 'TRANSPARENT':
            hex_data = msg_tuple[2] if len(msg_tuple) > 2 else ''
            b72_value = msg_tuple[7] if len(msg_tuple) > 7 else None
            f2 = msg_tuple[5] if len(msg_tuple) > 5 else 0
            return f"[Transparent] f2:{f2} b72:0x{b72_value:018x} | hex:{hex_data}"
        if msg_tuple and len(msg_tuple) >= 3:
            return ' '.join(str(x) for x in msg_tuple[:3] if x)
        return ''

    def get_transparent_data(self):
        if self.msg_tuple and len(self.msg_tuple) > 0 and self.msg_tuple[0] == 'TRANSPARENT':
            return {
                'format': self.msg_tuple[1] if len(self.msg_tuple) > 1 else 'UNKNOWN',
                'hex': self.msg_tuple[2] if len(self.msg_tuple) > 2 else '',
                'decimal': int(self.msg_tuple[3]) if len(self.msg_tuple) > 3 and self.msg_tuple[3].isdigit() else 0,
                'f2': self.msg_tuple[5] if len(self.msg_tuple) > 5 else 0,
                'bits74': self.msg_tuple[4] if len(self.msg_tuple) > 4 else 0,
                'bytes': self.msg_tuple[6] if len(self.msg_tuple) > 6 else '',
                'b72': self.msg_tuple[7] if len(self.msg_tuple) > 7 else 0
            }
        return None

    def export_transparent_to_file(self, filename):
        data = self.get_transparent_data()
...
class Receiver():
    ...
    def search(self, f0_idxs, cyclestart, sync_idx=1):
        ...
        costas_nhops = 7 * HPS
        edge_to_cent = BPT // 2
        with self.audio_in.lock:   # [FIX] Lock for reading
            search_hops = self.audio_in.dBgrid_main[
                cycle_h0 + H0_RANGE[0] + sync_idx_offs: cycle_h0 + H0_RANGE[1] + sync_idx_offs + costas_nhops,
                edge_to_cent:]
    ...
    def manage_cycle(self):
        ...
        print("Rx running (processing i3=0,1,6 messages)")
        ticker_cycle_rollover = Ticker(0)
        ticker_search_for_syncs = Ticker(H_SEARCH_1, timing_function=lambda: self.audio_in.dBgrid_main_ptr,
        ...
        while True:
            ...
            if ticker_search_for_syncs.ticked():
                global_time_utils.tlog(f"[Cycle manager] start search at hop {self.audio_in.dBgrid_main_ptr}",
                                       verbose=self.verbose)
                corrected_now = global_time_utils.corrected_time()
                cyclestart = global_time_utils.cyclestart(corrected_now)
                ...
                global_time_utils.tlog(f"[Cycle manager] New spectrum searched -> {len(candidates)} candidates",
                                       verbose=self.verbose)